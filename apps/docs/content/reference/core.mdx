...
```ts
import { createZip, EazipJobFailedError, isEazipError } from '@eazip/core';

try {
  await createZip({ files });
} catch (error) {
  if (error instanceof EazipJobFailedError) {
    console.error(error.failedCount, error.failures);
