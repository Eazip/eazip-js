...
| `zips` | The zip(s) produced so far — see [Multi-zip splitting](/docs/concepts/splitting). |
| `errors` / `skippedCount` | Per-file skips — see [Partial results & errors](/docs/concepts/partial-results). |
| `error` | The fatal error, set only when `status === 'failed'`. |
| `session` | Cloud only — `{ sessionId, clientSecret, ... }`, populated as soon as the session exists. See [Sessions & resume](/docs/concepts/resume). |
